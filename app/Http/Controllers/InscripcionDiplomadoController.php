...
use App\Models\InscripcionDiplomado;
...
class InscripcionDiplomadoController extends Controller
{
    public function store(Request $request, Q10Service $q10Service)
    {
        $validated = $request->validate([
            ...
            'curso.required' => 'Debe seleccionar un diplomado',
        ...
        try {
            $inscripcion = InscripcionDiplomado::create($validated);
    ...
    {
        $query = InscripcionDiplomado::query();
        ...
        $stats = [
            'total' => InscripcionDiplomado::count(),
            'pendientes' => InscripcionDiplomado::where('estado', 'pendiente')->count(),
            'contactados' => InscripcionDiplomado::where('estado', 'contactado')->count(),
            'matriculados' => InscripcionDiplomado::where('estado', 'matriculado')->count(),
        ];

        return view('admin.inscripciones-diplomados', compact('inscripciones', 'stats'));
    }

    public function updateEstado(Request $request, InscripcionDiplomado $inscripcion)
    {
        $request->validate([
    ...
    public function destroy(InscripcionDiplomado $inscripcion)
    {
        $inscripcion->delete();
